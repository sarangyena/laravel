        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @endif
                    <form method="POST" action="{{ route('a-employee.update', $employee) }}" id="empForm"
                        ...
                        <div class="columns-2 py-2">
                            ...
                            <div class="flex justify-evenly pt-10">
                                ...
                                @if ($employee->role === 'EMPLOYEE')
                                    <input type="radio" id="emp" name="role" value="EMPLOYEE" required checked>
                                ...
                                @elseif ($employee->role === 'ON-CALL')
                                    <input type="radio" id="emp" name="role" value="EMPLOYEE" required>
                                    <x-input-label for="emp" :value="__('EMPLOYEE')" />
                                    <input type="radio" id="on" name="role" value="ON-CALL" required checked>
                                    <x-input-label for="on" :value="__('ON-CALL')" />
                                @endif
                            </div>
                            <div class="mt-5">
                                ...
                                <x-text-input id="userName" class="block mt-1 w-full uppercase" type="text"
                                    name="userName" value="{{old('userName', $employee->userName)}}" readonly required/>
                            </div>
                        </div>
                        ...
                        <div class="mt-3 columns-3">
                            ...
                            <x-select-input id="job" name="job" class="mt-1" required>
                                <option selected value="{{old('job', $employee->job)}}">{{old('job', $employee->job)}}</option>
                            ...
                            <x-text-input id="sss" class="block mt-1 w-full uppercase" type="text"
                                name="sss" value="{{old('sss', $employee->sss)}}" />
                            ...
                            <x-text-input id="philhealth" class="block mt-1 w-full uppercase" type="text"
                                name="philhealth" value="{{old('philhealth', $employee->philhealth)}}" />
                        </div>
                        <div class="mt-3 columns-2">
                            ...
                            <x-text-input id="pagibig" class="block mt-1 w-full uppercase" type="text"
                                name="pagibig" value="{{old('pagibig', $employee->pagibig)}}" />
                            <x-input-label for="rate" :value="__('Rate:')" />
                            <x-text-input id="rate" class="block mt-1 w-full uppercase" type="text"
                                name="rate" value="430" value="{{old('rate', $employee->rate)}}" required/>
                        </div>
                        ...
                        <div class="flex mt-3 flex-row-reverse">
                            ...
                            <x-secondary-button onclick="window.location.href='{{ route('a-employee.index') }}'" class="m-3">
                                {{ __('Cancel') }}
                            </x-secondary-button>
                        </div>
                    </form>
